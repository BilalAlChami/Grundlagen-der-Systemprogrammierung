...
// Demnach hat ein Tag nur 3 Stunden (0 bis 2) und eine
// Stunde nur 6 Minuten (0 bis 5) und eine Minute 6 Sekunden (0 bis 5)
#define ZT_MAXW 2
#define SS_MAXW 6
#define MM_MAXW 6
#define HH_MAXW 3
...
void emain(void* arg) 
{
    ...
    // Zur Sicherheit vor Initialisierung den Interupt des PIC generell deaktivieren
       ...
       /*putstring("Bitte die aktuelle Uhrzeit im Format hh:mm:ss eingeben\n");
        unsigned char stringbufeingabe[100];
        getstring(stringbufeingabe);
        sscanf(stringbufeingabe,"%d:%d:%d",&akt_zeit.hh, &akt_zeit.mm, &akt_zeit.ss);
        */
...
void init_spi1(){
    ...
    // ...
    /*SPI1 ist Slave*/
        io_out8(SPCR1,((1<<SPE1)| (1<<SPIE1)));//SPE1:bekommt eine 1 um das SPI_schnittstelle des Slaves zu aktivieren
                                                // SPIE1: bekommt ien e1 um das SPI_interrupt des Slaves zu aktivieren

}

void ISR_SPI1_SlaveReceive(void){

    unsigned char buf1;


    byte_received=(io_in8(SPDR1));//Das register SPDR1 speichert das kommende 8 bits nachricht von dem Master

    // Zuruecksetzen des Interrupt-Flags damit wir eine neue nachricht empfangen kÃ¶nnen.weil bei jeder abgeschlossene Ã¼bertragung
    //eines bytes wird das SPI_Flag gesetzt
    buf1 = io_in16(SPSR1);
    buf1 = buf1 & ~(1 << SPIF1);// wird aktiv wenn die Übertragung ein Byteabgeschlossen ist
    io_out16(SPSR1, buf1);
}
...
void emain(void* arg)
{
    ...
    // ...
    //SPI-schnittstellen des Slaves initialisieren
            init_spi1();
            //ISR registrieren
            setInterruptHandler(IVN_SPI1, ISR_SPI1_SlaveReceive);
...
void init_spi2(){
    // Diese Funktion muss noch mit Leben gefuellt werden.
    // ...
    //SPI2ist Master , clock_rate=1/4
      io_out8(SPCR2,((1<<SPE2)| (1<<MSTR2)));//MSTR2=1   die SPI_schnittstelle wurde als Master konfiguriert
}

//funktion um das versanden eines bytes durchzufÃ¼hren
void SPI_MasterTransmit(char cdata){
    io_out(SPDR2,cdata);
    while(!((io_in8(SPSR2)) & (1<<SPIF2)));//while SPIF2==1 soll die nachricht cdata in dem register SPDR2 geschrieben
}
...
void emain_sender(void* arg)
 {
     ...
     // Hier die notwendigen Initialisierungen einfuegen.
     // ...
     //SPI-schnittstellen des Mesters initilisieren
          init_spi2();

          // Den Slave neu synchronisieren.
          // Dazu notSS zunÃ¤chst auf 1 und danach auf 0 setzen
          buf=in8(SPCR2);

          buf = buf | ( 1 << notSS2);//notSS=1 wurde zurÃ¼ck gesetzt
          io_out8(SPCR2, buf);
          buf=in8(SPCR2);

          buf = buf & ~( 1 << notSS2);//notSS=0 wurde gesetzt
          io_out8(SPCR2, buf);
     ...
     while(1) {
         ...
         do  {
             ...
             // ...
             SPI_MasterTransmit(parametriere_akt_zeit[i]);
         ...
         } while(parametriere_akt_zeit[i]!='\0');
